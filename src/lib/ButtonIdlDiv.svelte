<script>
  ...
  onMount(() => {
    buttonElem.role = 'button'

    // Report if browser did not apply role through attribute reflection
    if (!buttonElem.getAttribute('role')) {
      ariaReflectionSupport.set(false)
      return
    }

    // This is doable with the framework but this is for illustration/consistency with example
    buttonElem.addEventListener('click', activateActionButton)
    buttonElem.addEventListener('keydown', actionButtonKeydownHandler)
    buttonElem.addEventListener('keyup', actionButtonKeyupHandler)
  })
</script>

<div tabindex="0" {...$$props} bind:this={buttonElem}><slot /></div>
