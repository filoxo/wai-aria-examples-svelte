<script>
  ...
  const toggleButtonKeydownHandler = handleKeyboardClick(dispatchClick)
  ...
  onMount(() => {
    ...
    if (
      !anchorElem.getAttribute('role') ||
      !anchorElem.getAttribute('aria-pressed')
    ) {
      ariaReflectionSupport.set(false)
      return
    }

    // This is doable with the framework but this is for illustration/consistency with example
    anchorElem.addEventListener('click', toggleButtonClickHandler)
    anchorElem.addEventListener('keydown', toggleButtonKeydownHandler)
    anchorElem.addEventListener('keyup', toggleButtonKeyupHandler)
  })
</script>

<div tabindex="0" {...$$props} bind:this={anchorElem}><slot /></div>
